...
export const Filters = () => {
	...
	const countRemaining = () => {
		// This could be resolved on server side
		return todoList.filter(({isDone}: Todo) => !isDone).length;
	};
	...
	const typeFilter = (type: TodoListType, index: number) =>
		<a key={`${type}-${index}`} className={`cursor-pointer p-1 capitalize ${filter === type ? 'font-bold dark:font-normal dark:text-gray-200' : 'hover:text-gray-400 hover:dark:text-gray-200'}`}
	...
	return (
		<div className="flex p-2 sm:p-3 items-center justify-between border border-gray-200 rounded dark:border-gray-700 text-xs text-gray-600 dark:text-gray-500 bg-white  dark:bg-slate-800">
			...
			<div className="text-center sm:w-2/5">
				<a className="cursor-pointer hover:text-gray-400 hover:dark:text-gray-200" onClick={() => clearCompleted()}>Clear Completed</a>
			</div>
		</div>
	);
};
